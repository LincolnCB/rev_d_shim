...
if {$use_ext_clk} {
  ...
  cell xilinx.com:ip:clk_wiz:6.0 spi_clk_gen {
    ...
    locked hw_manager/spi_clk_locked
  }
  ## SPI clock control snoop to internally track SPI clock frequency
  #    Register default parameters are set to match those generated by the clock wizard above
  ...
  cell shim:user:axi_clock_timing_snoop spi_clk_snoop {
    ...
    aclk ps/FCLK_CLK0
    aresetn ps_rst/peripheral_aresetn
    reconf_in_prog hw_manager/spi_clk_reconf_in_prog
    div_by_zero hw_manager/spi_clk_div_by_zero
    freq_too_high hw_manager/spi_clk_freq_oob
    S_AXI sys_cfg_axi_intercon/M02_AXI
    M_AXI spi_clk_gen/s_axi_lite
  }
} else {
...
addr 0x40200000 2048 spi_clk_snoop/S_AXI ps/M_AXI_GP0
addr 0x00000000 2048 spi_clk_gen/s_axi_lite spi_clk_snoop/M_AXI
...
#  4  1183 : 1152 --  32b SPI source clock frequency in Hz
#  5  1215 : 1184 --  32b Trigger counter (number of triggers received)
#  6  1247 : 1216 --  32b Debug 1 (SPI clock locked, spi_off, DAC/ADC ~CS high time)
#  7  1279 : 1248 --  32b DAC minimum delay time (in SPI clock cycles)
#  8  1311 : 1280 --  32b ADC minimum delay time (in SPI clock cycles)
#  9  1567 : 1312 -- 256b DAC last received command (32 bits per channel, ordered as DAC0, ..., DAC7)
# 10  1823 : 1568 -- 256b ADC last received command (32 bits per channel, ordered as ADC0, ..., ADC7)
# 11  2079 : 1824 -- 256b DAC commands since reset  (32 bits per channel, ordered as DAC0, ..., DAC7)
# 12  2335 : 2080 -- 256b ADC commands since reset  (32 bits per channel, ordered as ADC0, ..., ADC7)
# 13  4095 : 2336 -- RESERVED (zero-filled)
cell xilinx.com:ip:xlconcat:2.1 sts_concat {
  ...
  In6 debug_1/dout
  In7 dac_min_delay_time_concat/dout
  In8 adc_min_delay_time_concat/dout
...
cell xilinx.com:ip:xlconstant:1.1 pad_sts_reserved {
  CONST_VAL 0
  CONST_WIDTH [expr {4096 - 2336}]
} {
  dout sts_concat/In13
}
