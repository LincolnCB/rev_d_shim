...
static bool parse_manual_set(parsed_command_t *out, parse_state_t *state) {
	...
	if (state->runtime_state == NULL || state->runtime_state->channel_count == 0U) {
		command_error(state->error_buf, state->error_buf_size, "channel count is not configured");
		return true;
	}
	if (channel < 0 || (uint32_t)channel >= state->runtime_state->channel_count) {
...
input_parse_result_t input_parse_line(const char *line,
	...
	// Try parsers in priority order; first match wins.
