...
async def test_set_lockout_cmd(dut):
    ...
    await RisingEdge(dut.clk)
    cmd_buf_task.cancel()
    monitor_cmd_done_task.cancel()
    monitor_state_transitions_task.cancel()
    scoreboard_executing_cmd_task.cancel()
    trig_timer_task.cancel()
    data_buf_task.cancel()
    data_buf_scoreboard_task.cancel()
...
async def test_unexpected_cmd(dut):
    ...
    # Command with an invalid command type (0 is the only unexpected type; 6 is CMD_RESET_COUNT)
    cmd_list.append(tb.command_word_generator(0, 0x1234567))
...
async def test_random_cmd_sequence(dut):
    ...
    for i in range(100):
        ...
        # Generate a random sequence of commands. Prepend a small valid SET_LOCKOUT so that
        # EXPECT_EXT_TRIG commands run at the minimum lockout instead of the 10M-cycle default
        # (reset restores trig_lockout to TRIGGER_LOCKOUT_DEFAULT each iteration).
        cmd_list = [tb.command_word_generator(2, tb.TRIGGER_LOCKOUT_MIN)] + tb.random_command_word_generator(49)
        ...
        data_buf_task.cancel()
        data_buf_scoreboard_task.cancel()
        waiting_for_trig_task.cancel()
        ext_trig_task.cancel()
