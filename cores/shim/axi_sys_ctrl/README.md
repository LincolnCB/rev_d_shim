...
| 0x2C           | 11            | `datapath_mode`        | 8 bits  | Per-board high-rate datapath select (bit b: 0 = PIO, 1 = DMA) | 0     | 0 to 0xFF, locked by `ctrl_en` |
...
- **Outputs**:
  ...
  - `datapath_mode`: 8-bit per-board high-rate datapath select mask.
  - Out-of-bounds signals: `ctrl_en_oob`, `pow_en_oob`, `cmd_buf_reset_oob`, `data_buf_reset_oob`, `thresh_val_oob`, `thresh_window_oob`, `thresh_en_oob`, `boot_test_skip_oob`, `debug_oob`, `dac_cal_init_oob`, `do_dac_pre_delay_oob`, `datapath_mode_oob`.
...
- The `datapath_mode` register selects each board's high-rate datapath (DAC command and ADC data lanes): bit `b` low routes board `b` through the programmed-I/O (PIO) FIFO bridges -- direct CPU register access -- and bit `b` high routes it through the MCDMA. It resets to all-PIO so power-on reproduces the non-DMA datapath, and is locked by `ctrl_en` so a board's path cannot change mid-run.
