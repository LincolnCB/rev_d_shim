...
Eight boards give 8 MM2S + 8 S2MM, which is exactly one MCDMA. The other per-board FIFOs are low-throughput and their current shallow buffers are adequate, so they keep the existing `axi_fifo_bridge` programmed-I/O (PIO) interface -- the PS reaching a FIFO by direct CPU register access, its window mapped once with `mmap()` -- unchanged:
...
## Coexistence of the PIO and DMA paths
...
`tlast` is the real design, and it is resolved as **adaptive word-granular packetization**. An earlier plan tied packet boundaries to the 4-word sample-set -- one full read op captures all eight channels as eight 16-bit SPI words packed into four 32-bit FIFO words -- but that alignment does not hold: a single-channel read (`ADC_RD_CH`) writes just one word, so the lane mixes 4-word and 1-word reads with no fixed boundary. Packet framing therefore carries no sample-set structure: a packet may close on any word, and a read may be split across packets. Software reassembles the samples from the captured buffer by position, so a split is harmless and an unexpected break is handled in software rather than in hardware. The number of words per run is highly variable, so packet size is not fixed: a small packetizer core drains the words currently resident and closes the packet (asserts `tlast`) when the FIFO is about to run dry or the packet reaches a cap.

Why adaptive rather than a fixed size: the packet-atomic mux holds a granted board until it sees `tlast`, so a fixed chop strands the mux mid-packet whenever the granted board's FIFO momentarily runs dry -- head-of-line blocking that stalls every other board and risks the ADC-overflow shutdown. Ending the packet exactly when the board's available data runs out releases the mux the instant that board has nothing more ready. It also self-flushes the tail of a run: when the ADC stops and the FIFO drains, the drain condition closes the final packet on its own, so `tlast` needs no separate end-of-sequence signal. A slow trickle where the reader keeps pace with the writer simply yields single-word packets.

The signal this rests on already exists: `fifo_async` exposes `fifo_count_rd_clk`, the read-side fill count, in the same 100 MHz AXI domain the mux runs in. The count derives from a Gray-synchronized pointer, so it can lag but never over-report -- the safe direction (worst case, a packet closes one word early). The packetizer forwards a word on every accepted beat, asserts `tlast` when this is the last resident word (`fifo_count_rd_clk <= 1`) or the packet has reached `MAX_PACKET_WORDS`, and drives a full AXIS master bundle (`tdata`, `tvalid`, `tready`, `tlast`, `tdest`) to the mux. It replaces `axis_fifo_bridge` on the ADC read side, since the framing decision needs the FIFO count and read-enable together and `axis_fifo_bridge` is `tdata`-only.

`MAX_PACKET_WORDS` has one hard ceiling: it must stay at or under the mux's `ARB_ON_MAX_XFERS` backstop (1024 beats in ex05), or the mux re-arbitrates mid-packet and breaks packet-atomicity -- the ex05 corruption bug. That ceiling is comfortable: at 1024 beats a board holds the port for roughly 10 us, during which each other board (2 MB/s, about one word per 2 us) accumulates only about 5 words, negligible against even a shrunk elastic FIFO. So a generous cap gives large, descriptor-cheap packets under load with no real fairness cost.

Because packets carry no sample-set framing, the S2MM capture is a flat word stream that software slices back into reads -- see the debug-lane cleanup below for the one thing that must stay off that stream.

## Debug lanes: cleanup required before completion

The ADC debug words (`ADC_DBG_*` codes in the top nibble) are currently interleaved into the `adc_data` lane -- a post-hoc hack that shared the lane to keep FIFO utilization low. The packetizer no longer assumes any sample-set alignment, but interleaved debug words would still land in the DMA capture buffer as non-sample data mixed into the ADC stream, so the debug stream must move onto its own dedicated ADC debug lane before this integration is considered complete. In normal operation debug is disabled and the `adc_data` lane carries only sample words, so the port proceeds on that assumption for now, but the split is a required closing task, not optional.

The DAC side is the mirror image: `dac_data_fifo` is really only a readback/debug lane, so it should be renamed `dac_debug` to reflect what it is. Both debug lanes stay on the PIO path -- they are low-throughput and not part of the DMA datapath. The original motivation for sharing lanes (BRAM pressure) goes away once DMA lets the data FIFOs shrink, so the clean split is affordable exactly when this work lands.
...
**Stage 2 -- PL datapath (MCDMA plus routing), PIO path preserved.** Add the `axi_mcdma` (8 MM2S + 8 S2MM), an `axis_switch` demux fanning MM2S to the eight DAC-command `axis_fifo_bridge` instances by `tdest`, and a packet-atomic `axis_switch` mux recombining the eight ADC-data streams into S2MM. Reuse the ex05 mux settings that were hard-won there (`HAS_TLAST` and `ARB_ON_TLAST` set together, the `tdest` window in `0x` hex form, `ARB_ON_MAX_XFERS` backstop). Add the ADC packetizer core (adaptive word-granular off `fifo_count_rd_clk`, `MAX_PACKET_WORDS` at or under the mux backstop), the datapath-mode mux against the existing `axi_fifo_bridge`, the HP0 64-bit master, and the 23-bit buffer-length register. Fold the sixteen MCDMA completion/error `introut` lines into `hw_manager` -- it aggregates them into its state machine and status word and keeps raising its single `ps_interrupt`, so DMA completion and faults ride `hw_manager`'s single interrupt (delivered non-root via `pl-irq`, per Stage 1) rather than a separate one. After a build and boot, confirm a byte-exact loopback-style transfer on the DMA-driven FIFOs, and confirm the mmap fallback still works with the mode bit set to PS.
...
**Stage 5 -- Validation and reclaim.** With the datapath proven, reclaim resources: shrink the DMA-driven CDC FIFOs from whole-waveform depth toward microsecond elastic buffers (freeing most of the BRAM they hold today), and drop redundant PS-side bridges if the PIO fallback is retired. Split the ADC debug words onto a dedicated debug lane and rename `dac_data` to `dac_debug` (see Debug lanes) -- the shrunk buffers make this affordable, and it is required before the integration is complete.

LUT is the binding constraint at eight boards, so the reclaim pass targets it directly. On the four-board bring-up build the design is 33,859 LUT (~64% of the 53,200), which extrapolates to roughly 96% at eight boards; the scaling is dominated by the per-board DAC controllers (~1,910 LUT each: ~1,356 for the SPI engine plus calibration, ~532 for the threshold core) and the per-board AXI-Lite interconnect fabric, not the DMA datapath itself, whose adapters, packetizer, and select add well under 100 LUT per board. The reclaim knobs, roughly in order of return over risk:

- Replace the memory-mapped AXI4-Lite SmartConnects (`board_ch_axi_intercon`, the per-board `ch*_axi_intercon`, `sys_cfg_axi_intercon`) with a lighter `axi_interconnect` in shared-lite mode or a small custom address decoder. SmartConnect is a full AXI4 crossbar and is heavily oversized for single-beat register and FIFO windows, so this is the largest no-behavior-change win.
- Lean the HP0 SmartConnect (`axi_mem_intercon`, ~4,400 LUT): strip register slices and cut the outstanding/buffer depth, or route the small SG descriptor fetches through a GP port so it arbitrates two masters into HP0 instead of three.
- Trim the continuously cross-domain-synced telemetry (`spi_sts_sync`, ~1,600 LUT at four boards): the per-channel "last received command" and "commands since reset" arrays are observability, not function -- gate them behind debug mode or capture them on a read handshake rather than every cycle.
- Consolidate the per-board PIO bridges once DMA is proven: after the switch each per-board bridge is half-idle (only `adc_cmd` writes and `dac_debug` reads remain on PIO), so collapse each board to one narrow bridge for those lanes and shrink the interconnect feeding them.
- Move DAC calibration into the software-prebuffered command stream: since DMA prebuffers the DAC commands in DDR, software can apply per-channel calibration when it builds the buffer, letting the PL DAC controller drop its calibration adder and offset conversion. This is the largest structural saving on the dominant block, but it changes the calibration flow and is only clean once the PIO DAC path is retired.
- The threshold core (~532 LUT per board) is over-current safety logic; time-multiplexing one comparator across a board's eight channels, or narrowing it, reclaims part of it, but it must be scrutinized as a safety path rather than trimmed casually.

These are a deliberate later pass: the four-board bring-up validates the datapath first, and the reclaim work follows once the design is settled. Update this document to reflect the integrated state.
...
Every design question is settled. ADC stream framing is adaptive word-granular packetization off `fifo_count_rd_clk`, which self-flushes the run tail; packets carry no sample-set alignment (a single-channel `ADC_RD_CH` read writes one word, so 4-word atomicity does not hold) and software recombines reads from the capture buffer (see ADC stream framing and packetization). Datapath-mode control is a per-board `datapath_mode` mask in `axi_sys_ctrl`, reset to PIO, with a `mode_viol` fault routed through `hw_manager` (see Coexistence). DMA completion and faults fold into `hw_manager`'s single interrupt, delivered non-root via `pl-irq` and consumed by an event-driven run-controller (see Software: event-driven run control). The port migrates rev_d off root `/dev/mem` onto `pl-reg` / `pl-irq` (Stage 1). The `reserved-memory` size is fixed at 64 MB, held as a single `#define` in the project device tree (see Reserved memory). No open questions remain.
